...
import { API_BASE_URL, ServiceMode } from "../constants/config";

const ACCESS_TOKEN_KEY = "driver_access_token";
const REFRESH_TOKEN_KEY = "driver_refresh_token";

// Web-compatible storage wrapper (expo-secure-store has no web implementation)
const storage = {
...
export interface DriverProfile extends Driver {
  ...
  driver?: {
    ...
    isOnline: boolean;
    nightMode: boolean;  // Section 20: Night mode opt-in
    rating: number;
    totalTrips: number;
...
class DriverAuthService {
  ...
  async getMe(): Promise<DriverProfile | null> {
    try {
  ...
  async updateProfile(updates: { name?: string; vehicleType?: string; vehiclePlate?: string }): Promise<boolean> {
    try {
      const response = await this.authenticatedFetch(`${API_BASE_URL}/auth/me`, {
        method: "PATCH",
  ...
  // Dev-only login for testing without OTP
